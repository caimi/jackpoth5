...
function moveBalls(){
	for(var i=0; i<UJAPP.players.length; i++){
		var ball = UJAPP.players[i].ball;
		
		if(ball.p.x < 200 + ball.r){		
			ball.v.x = -ball.v.x;
			ball.p.x = 200 + ball.r;
		}
		if(ball.p.x > (UJAPP.W-21 - ball.r)){
			ball.v.x = -ball.v.x;
			ball.p.x = (UJAPP.W-21 - ball.r);	
		}
		if(ball.p.y < 70+ ball.r){
			ball.v.y = -ball.v.y;
			ball.p.y = 70+ ball.r;		
		}
		if(ball.p.y > UJAPP.H-102 - ball.r){
			ball.v.y = -ball.v.y;
			ball.p.y = UJAPP.H-102 - ball.r;
		}
